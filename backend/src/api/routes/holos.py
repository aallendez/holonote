...
def get_holo_config_route(db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Unexpected error while fetching holo config: {str(e)}")


@router.put("/holo", response_model=Holo)
def update_holo_config_route(holo: HoloUpdate, db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Database error while updating holo config: {str(e)}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Unexpected error while updating holo config: {str(e)}")


@router.post("/holo", response_model=Holo)
def create_holo_config_route(holo: HoloCreate, db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Database error while creating holo config: {str(e)}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Unexpected error while creating holo config: {str(e)}")


@router.get("/daily", response_model=HoloDaily, description="Get the holo daily by date for a user")
def get_holo_daily_route(entry_date: date, db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Database error while fetching holo daily: {str(e)}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Unexpected error while fetching holo daily: {str(e)}")


@router.get("/daily/latest", response_model=HoloDaily)
def get_latest_holo_daily_route(db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Database error while fetching latest holo daily: {str(e)}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Unexpected error while fetching latest holo daily: {str(e)}")


@router.post("/daily", response_model=HoloDaily)
def create_holo_daily_route(holo_daily: HoloDailyCreate, db: Session = Depends(get_db), user=Depends(get_current_user)):
