...
import { getHoloConfig, createHoloConfig, createHoloDaily, type HoloConfig, type HoloDailyCreate } from '../api/holos';
...
export function HoloPopup({ isOpen, onClose, onComplete }: HoloPopupProps) {
  ...
  const loadHoloConfig = async () => {
    ...
    try {
      ...
      if (err instanceof Error && err.message.includes('No holo configuration found')) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(`Failed to submit your holo daily: ${err instanceof Error ? err.message : 'Unknown error'}`);
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="fixed inset-0 flex items-center justify-center p-4 z-[9999]">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              <div className="text-center">
                ...
                <div className="text-xs text-gray-500 dark:text-gray-400">Today's Score</div>
              </div>
              <button
                onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto max-h-[60vh]">
          {loading ? (
            ...
            <div className="text-center py-8">
              <div className="text-gray-600 dark:text-gray-300 mb-4">No questions available</div>
              <button
                onClick={loadHoloConfig}
                className="px-4 py-2 bg-black text-white rounded-lg hover:bg-gray-800 transition-colors"
              >
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                {questions.map((question, index) => (
                  <div key={index} className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-gray-200 dark:border-gray-700 hover:border-blue-300 dark:hover:border-blue-600 transition-colors">
                    <div className="flex items-center justify-between">
          ...
          <div className="p-6 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
            <div className="flex items-center justify-between">
              <div className="text-sm text-gray-600 dark:text-gray-300">
                {Object.keys(answers).length} of {questions.length} questions answered • Score: {dailyScore}/10
              </div>
              <div className="flex space-x-3">
                ...
                >
                  {submitting ? (
                    <LoadingSpinner label="Submitting..." size={16} className="text-white" />
                  ) : (
                    'Submit Holo'
                  )}
                </button>
