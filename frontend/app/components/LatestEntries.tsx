    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white/60 dark:bg-gray-900/60 backdrop-blur p-4">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="text-xs cursor-not-allowed text-gray-600 dark:text-gray-300 group hover:underline mr-2"
        >
          See all entries (coming soon...){" "}
          <p className="inline-block ml-1 transition-transform -rotate-45 group-hover:translate-x-1 group-hover:-translate-y-1">
            →
          </p>
        </button>
      </div>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              ...
              {latest.map((e) => (
                <tr
                  ...
                  onClick={() =>
                    navigate(`/entries?entry=${encodeURIComponent(e.entry_id)}`)
                  }
                >
                  <td className="py-3 pr-4 font-medium truncate max-w-[280px] text-gray-900 dark:text-gray-100">
                    {e.title}
                  </td>
                  <td className="py-3 pr-4 whitespace-nowrap text-gray-600 dark:text-gray-300">
                    {new Date(e.created_at).toLocaleDateString()}
                  </td>
                  <td className="py-3 pl-2 text-gray-400 group-hover:text-gray-600 dark:group-hover:text-gray-300 group-hover:translate-x-1 transition-transform">
                    →
                  </td>
                </tr>
              ))}
